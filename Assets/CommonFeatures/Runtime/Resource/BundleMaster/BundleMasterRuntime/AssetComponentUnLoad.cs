...
namespace BundleMaster
{
    public static partial class AssetComponent
    {
        ...
        public static void UnLoadByPath(string assetPath, string bundlePackageName = null)
        {
            ...
            {
                CommonLog.Resource("AssetLoadMode = Develop 不需要卸载");
                return;
            }
            ...
            {
                CommonLog.ResourceError("没有找到这个分包: " + bundlePackageName);
                return;
            }
            if (!bundleRuntimeInfo.AllAssetLoadHandler.TryGetValue(assetPath, out LoadHandler loadHandler))
            {
                CommonLog.ResourceError("卸载没有找到这个资源的Handler: " + assetPath);
                return;
            }
            loadHandler.UnLoad();
        }
        ...
        public static void UnLoadPackageAssets(string bundlePackageNames)
        {
            ...
            {
                CommonLog.Resource("没有找到这个初始化的分包: " + bundlePackageNames);
                return;
            }
        ...
        public static void UnInitialize(string bundlePackageName)
        {
            if (AssetComponentConfig.AssetLoadMode == EAssetLoadMode.Editor)
            {
                CommonLog.Resource("AssetLoadMode = Develop 无法取消初始化分包");
                return;
            }
        ...
        private static void UnInitializePackage(string bundlePackageName)
        {
            ...
            {
                CommonLog.ResourceError("找不到要取消初始化的分包: " + bundlePackageName);
                return;
            }
