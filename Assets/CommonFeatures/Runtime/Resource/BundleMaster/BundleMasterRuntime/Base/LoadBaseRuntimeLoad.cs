...
namespace BundleMaster
{
    public partial class LoadBase
    {
        ...
        internal void SubRefCount()
        {
            ...
            {
                CommonFeatures.Log.CommonLog.ResourceError("资源未被加载，引用不可能减少\n" + FilePath);
                return;
            }
            if (_loadState == LoadState.Loading)
            {
                CommonFeatures.Log.CommonLog.Resource("资源加载中，等加载完成后再进入卸载逻辑\n" + FilePath);
                return;
            }
            if (_refCount <= 0)
            {
        ...
        internal void LoadAssetBundle(string bundlePackageName)
        {
            ...
            if (_loadState == LoadState.Loading)
            {
                CommonLog.ResourceError("同步加载了正在异步加载的资源, 打断异步加载资源会导致所有异步加载的资源都立刻同步加载出来。资源名: " + FilePath + 
        ...
        /// </summary>
        private async UniTask LoadDataFinish(string assetBundlePath, char[] bundlePackageSecretKey)
        {
            ...
            await UniTask.WaitUntil(() => _assetBundleCreateRequest.isDone);
            AssetBundle = _assetBundleCreateRequest.assetBundle;
            //判断是否还需要
            if (_refCount <= 0)
            {
        ...
        internal void ForceLoadFinish(string bundlePackageName)
        {
            ...
            if (_assetBundleCreateRequest != null)
            {
                CommonLog.ResourceError("触发强制加载, 打断异步加载资源会导致所有异步加载的资源都立刻同步加载出来。资源名: " + FilePath + 
                                       "\nAssetBundle包名: " + AssetBundleName);
                AssetBundle = _assetBundleCreateRequest.assetBundle;
                return;
            }
        ...
        internal float GetProgress()
        {
            ...
            {
                CommonLog.ResourceError("未打开进度统计无法获取进度");
                return 0;
            }
            if (_loadState == LoadState.Finish)
            {
