...
namespace BundleMaster
{
    public static class DownloadBundleHelper
    {
        public static async UniTask<byte[]> DownloadDataByUrl(string url)
        {
            ...
            CommonLog.ResourceError("下载资源失败: " + url);
            return null;
        }
        ...
        private static async UniTask<byte[]> DownloadData(string url)
        {
            using (UnityWebRequest webRequest = UnityWebRequest.Get(url))
            {
                ...
                cts.CancelAfterSlim(5000);
                var (isCanceled, webrequest) = await webRequest.SendWebRequest()
                    .WithCancellation(cts.Token)
                    .SuppressCancellationThrow();

                if (isCanceled)
                {
                    CommonLog.Resource("下载Bundle失败 重试\n 下载超时" + "\nURL：" + url);
                    return null;
                }
                else
                {
        ...
        public static async UniTask<DownLoadData> DownloadRefDataByUrl(string url)
        {
        ...
        private static async UniTask DownloadData(string url, DownLoadData downLoadData)
        {
            using (UnityWebRequest webRequest = UnityWebRequest.Get(url))
            {
                var cts = new CancellationTokenSource();
        ...
        /// 下载完成回调位
        /// </summary>
        internal static readonly Queue<UniTask> DownLoadFinishQueue = new Queue<UniTask>();
        ...
        /// 多线程下载资源直存
        /// </summary>
        public static async UniTask<LmtDownloadInfo> DownloadData(string url, string filePath, UpdateBundleDataInfo updateBundleDataInfo)
        {
        ...
        private static async UniTask<LmtDownloadInfo> DownloadOneData(string url, string filePath, UpdateBundleDataInfo updateBundleDataInfo)
        {
            ...
            await UniTask.WaitUntilCanceled(cancellation.Token);
            ...
            if (lmtDownloadInfo.LmtDownloadResult != LmtDownloadResult.Success)
            {
                ...
                {
                    CommonLog.ResourceError("下载资源失败: " + lmtDownloadInfo.LmtDownloadResult);
                }
            }
            lmtDownLoad.Dispose();
            return lmtDownloadInfo;
