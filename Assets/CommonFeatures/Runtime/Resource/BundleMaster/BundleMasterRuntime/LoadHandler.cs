    ...
    public class LoadHandler : LoadHandlerBase
    {
        ...
        internal void Init(string assetPath, string bundlePackageName, bool haveHandler)
        {
            ...
            if (groupPath != null)
            {
                ...
                {
                    CommonLog.ResourceError("没有找到资源组: " + groupPath);
                    return;
                }
                LoadBase = loadGroup;
                //需要记录loadGroup的依赖
                for (int i = 0; i < loadGroup.DependFileName.Count; i++)
                {
                    ...
                    CommonLog.ResourceError("依赖的资源没有找到对应的类: " + dependFile);
                }
                return;
            }
            //先找到对应加载的LoadFile类
            ...
            {
                CommonLog.ResourceError("没有找到资源: " + AssetPath);
                return;
            }
            LoadBase = loadFile;
            //需要记录loadFile的依赖
        ...
        internal async UniTask LoadAsync()
        {
            ...
            tasks.Add(LoadAsyncLoader(LoadBase));
            for (int i = 0; i < LoadDepends.Count; i++)
            {
                tasks.Add(LoadAsyncLoader(LoadDepends[i]));
            }
            for (int i = 0; i < LoadDependFiles.Count; i++)
            {
                tasks.Add(LoadAsyncLoader(LoadDependFiles[i]));
            }
            for (int i = 0; i < LoadDependGroups.Count; i++)
            {
                tasks.Add(LoadAsyncLoader(LoadDependGroups[i]));
            }
            await UniTask.WhenAll(tasks);
            if (LoadState == LoadState.Finish)
            {
                CommonLog.Resource("此资源异步加载时触发了强制加载: " + AssetPath);
            }
            LoadState = LoadState.Finish;
            FileAssetBundle = LoadBase.AssetBundle;
        }
        ...
        protected override void ClearAsset()
        {
            ...
            {
                CommonLog.ResourceError("没要找到分包的信息: " + BundlePackageName);
                return;
            }
            if (HaveHandler)
            {
                ...
                {
                    CommonLog.ResourceError("没要找到缓存的LoadHandler: " + AssetPath);
                    return;
                }
