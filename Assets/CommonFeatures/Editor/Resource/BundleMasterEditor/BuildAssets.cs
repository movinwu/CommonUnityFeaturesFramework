...
namespace BundleMaster
{
    public class BuildAssets
    {
        ...
        {
            var sw = CommonLog.StartTimeWatch("构建AssetBundle");
            AssetLoadTable assetLoadTable = AssetDatabase.LoadAssetAtPath<AssetLoadTable>("");
            ...
            //打包结束
            CommonLog.StopTimeWatch(sw, "构建AssetBundle");
            CommonLog.Resource($"打包结束\n{assetLoadTable.BuildBundlePath}");
        }
        ...
        private static void Build(AssetLoadTable assetLoadTable, AssetsLoadSetting assetsLoadSetting, HashSet<string> assetLoadPath, HashSet<string> alwaysIncludedShaders)
        {
            ...
            {
                CommonLog.ResourceError("没有资源: " + assetsLoadSetting.BuildName);
                return;
            }
            //保存打包Log
