...
namespace CommonFeatures.Test
{
    public class TestDataTable : MonoBehaviour
    {
        private void Start()
        {
            CommonFeaturesManager.DataTable.ReadDataTable();
            var datas = CommonFeaturesManager.DataTable.GetDataRows<DR_Example>();
            for (int i = 0; i < datas.Count; i++)
            {
                CommonLog.Config(LitJson.JsonMapper.ToJson(datas[i]));
                CommonLog.Config(datas[i].string_example);
            }
            var datas2 = CommonFeaturesManager.DataTable.GetDataRows<DR_Example2>();
